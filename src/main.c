...
static uint8_t m_plain_text[NRF_CRYPTO_EXAMPLE_AES_MAX_TEXT_SIZE] = {
	"Example string to demonstrate basic usage of AES CBC mode.dsdsdsdsdsdsdsdsdssdsdsdsdsddsdsdsdsdsdsdsdsds SURENDRA"
};
...
static int encrypt_cbc_aes(void)
{
	...
	LOG_INF("Time taken to encrypt the message: %lld ms", (end_time - start_time));
	LOG_INF("Time taken to encrypt the message: %lld ticks. Ticks per seonds %lld ", (end_tick - start_tick), sys_clock_hw_cycles_per_sec());
